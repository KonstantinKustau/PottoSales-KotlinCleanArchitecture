...
@Inject constructor(
    ...
    override fun signIn(email: String, password: String): Either<Failure, UserLoginPropertiesEntity> =
        loginRequest(email, password)

    override fun signUp(email: String, password: String): Either<Failure, UserLoginPropertiesEntity> =
        loginRequest(email, password)

    private fun loginRequest(
        email: String,
        password: String
    ): Either<Failure, UserLoginPropertiesEntity> =
        when (networkHandler.isConnected) {
            true -> request(
                service.signUp(email, password),
                { userPropertiesEntity -> userPropertiesEntity.toUserLoginProperties() },
                UserLoginProperties.empty()
            )
